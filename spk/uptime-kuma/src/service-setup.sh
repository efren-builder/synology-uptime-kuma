...
# ---------------------------------------------------------------------------
# Set defaults for variables DSM may not provide
# ---------------------------------------------------------------------------
SYNOPKG_PKGNAME="${SYNOPKG_PKGNAME:-uptime-kuma}"
SYNOPKG_PKGDEST="${SYNOPKG_PKGDEST:-/var/packages/uptime-kuma/target}"
SYNOPKG_PKGVAR="${SYNOPKG_PKGVAR:-/var/packages/uptime-kuma/var}"
PID_FILE="${PID_FILE:-/var/packages/uptime-kuma/var/uptime-kuma.pid}"
LOG_FILE="${LOG_FILE:-/var/packages/uptime-kuma/var/uptime-kuma.log}"

# ---------------------------------------------------------------------------
# Bundled Node.js (included in the package since DSM lacks Node.js >= 20)
# ---------------------------------------------------------------------------
NODE="${SYNOPKG_PKGDEST}/node/bin/node"
NPM="${SYNOPKG_PKGDEST}/node/bin/npm"

# ---------------------------------------------------------------------------
# Application paths
# ---------------------------------------------------------------------------
UPTIME_KUMA_DIR="${SYNOPKG_PKGDEST}/uptime-kuma"
...
# Add bundled Node.js to PATH for child processes
export PATH="${SYNOPKG_PKGDEST}/node/bin:${PATH}"
...
# ---------------------------------------------------------------------------
# validate_preinst — Pre-installation validation
#   Runs before install or upgrade. Return 1 to abort.
# ---------------------------------------------------------------------------
validate_preinst() {
    # Note: Node.js is bundled inside the package and won't be available until
    # after extraction (postinst). Only check pre-conditions here.

    # Check available disk space (need at least 500 MB for install)
    if [ -n "${SYNOPKG_PKGDEST_VOL}" ]; then
        AVAIL_KB=$(df -k "${SYNOPKG_PKGDEST_VOL}" 2>/dev/null | awk 'NR==2 {print $4}')
        REQUIRED_KB=512000  # ~500 MB
        if [ -n "${AVAIL_KB}" ] && [ "${AVAIL_KB}" -lt "${REQUIRED_KB}" ] 2>/dev/null; then
            echo "Insufficient disk space. At least 500 MB is required for Uptime Kuma."
            echo "Available: $((AVAIL_KB / 1024)) MB on ${SYNOPKG_PKGDEST_VOL}"
            return 1
        fi
    fi
...
service_prestart() {
    ...
    # Verify bundled Node.js is still accessible
    if [ ! -x "${NODE}" ]; then
        log_msg "service_prestart: ERROR - Node.js not found at ${NODE}"
        echo "Bundled Node.js is not available. The package may be corrupted — try reinstalling." >&2
        return 1
    fi
...
service_postupgrade() {
    ...
    # Ensure correct permissions on restored data
    chmod 750 "${SYNOPKG_PKGVAR}" 2>/dev/null
